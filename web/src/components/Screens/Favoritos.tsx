...
export const Favoritos = () => {
  ...
  const getUserLikes = async () => {
    await appService
      .getProfileLikes({ page: 1, limit: 6 })
  ...
  const changePage = async (pageIncrement: number) => {
    await appService
      .getProfileLikes({ page: page + pageIncrement, limit: 6 })
      .then((res: FetchRecipes) => {
        let response: FetchRecipes = res as any;
        setRecipes(response.data);
        setCursor(response.cursor);
  ...
  return (
    <>
      ...
      <div
        style={{ margin: "auto", paddingTop: "15px", marginTop: "5pt" }}
        className="cursor"
      >
        <span onClick={() => cursor?.previous_page != null && changePage(-1)}>
          <ArrowCircleLeftIcon
            fontSize="large"
            sx={{ color: cursor?.previous_page != null ? "#507DBC" : "gray" }}
          />
        </span>
        <span onClick={() => cursor?.next_page != null && changePage(1)}>
          <ArrowCircleRightIcon
            fontSize="large"
            sx={{ color: cursor?.next_page != null ? "#507DBC" : "gray" }}
          />
        </span>
      </div>
    </>
